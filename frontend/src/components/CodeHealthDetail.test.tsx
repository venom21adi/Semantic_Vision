...
import { CodeHealthDetail } from './CodeHealthDetail'
...
function renderDetail(overrides: Partial<React.ComponentProps<typeof CodeHealthDetail>> = {}) {
  const props: React.ComponentProps<typeof CodeHealthDetail> = {
...
describe('CodeHealthDetail', () => {
  it('renders the summary stats header when loaded on the Complexity tab', () => {
    renderDetail()
  ...
  it('does not render complexity-specific controls on the Hotspots tab', () => {
    renderDetail({ healthTab: 'hotspots' })

    expect(screen.queryByText('Functions scored')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Compare to last look' })).not.toBeInTheDocument()
  })

  it('renders the mini call graph pane for the selected function', () => {
    renderDetail({ selectedNodeId: 'app.py::handler' })

    expect(screen.getByTestId('rf__node-app.py::handler')).toBeInTheDocument()
  })

  it('captions the relationship graph with the selected function\'s pretty name', () => {
    renderDetail({ selectedNodeId: 'app.py::handler' })

    expect(screen.getByText(/Call relationships for/)).toBeInTheDocument()
    expect(screen.getAllByText('handler').length).toBeGreaterThan(0)
  })
  ...
  it('disables Compare while the complexity fetch itself is still loading', () => {
    renderDetail({ state: { status: 'loading' } })
  ...
  it('disables Compare and shows a comparing label while a last-look compare is in flight', () => {
    renderDetail({ diff: { status: 'loading', mode: { kind: 'last-look' } } })
  ...
  it('shows a diff error message', () => {
    renderDetail({ diff: { status: 'error', mode: { kind: 'last-look' }, message: 'diff boom' } })
  ...
  it('shows a no-baseline note when the diff is unavailable', () => {
    ...
    renderDetail({ diff: { status: 'loaded', mode: { kind: 'last-look' }, result } })
  ...
  it('renders added, removed, and changed sections and only removed rows are non-clickable', async () => {
    ...
    const { props } = renderDetail({ diff: { status: 'loaded', mode: { kind: 'last-look' }, result } })
  ...
  it('does not render the ref picker "Compare" button when the loaded repo is not a git repo', () => {
    renderDetail({ gitRefs: { status: 'loaded', refs: { is_git_repo: false, branches: [], commits: [] } } })
  ...
  it('scopes the relationship diagram to the selected function, not the whole repo', () => {
    ...
    expect(screen.getByTestId('rf__node-app.py::a')).toBeInTheDocument()
  ...
  it('shows the scoped relationship diagram on the Hotspots tab too', () => {
    renderDetail({ healthTab: 'hotspots', selectedNodeId: 'app.py::handler' })

    expect(screen.getByTestId('rf__node-app.py::handler')).toBeInTheDocument()
  })

  it('shows the scoped relationship diagram on the Dead code tab too', () => {
