...
describe('RefPicker', () => {
  ...
  it('calls onCompare with the typed ref and its resolved label', async () => {
    ...
    expect(onCompare).toHaveBeenCalledWith('abc1234', 'abc1234 fix the thing')
  })
  ...
  it('calls onCompare with the raw value as its own label for an arbitrary typed ref', async () => {
    ...
    expect(onCompare).toHaveBeenCalledWith('deadbee', 'deadbee')
  })

  it('disables the Compare button while the input is empty', () => {
    const state: GitRefsState = { status: 'loaded', refs: gitRepoRefs }
    render(<RefPicker state={state} disabled={false} onCompare={vi.fn()} />)

    expect(screen.getByRole('button', { name: 'Compare to commit' })).toBeDisabled()
  })
